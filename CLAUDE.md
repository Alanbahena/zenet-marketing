...
- **`04-go-to-market/12-proceso-de-ventas-fase-a.md` v0.1 CERRADO (15-sep) — el tríptico completo: ICP (doc 11) → landing v4 → ventas (doc 12).** Escrito sección por sección sobre la estructura del fundador: **script ensayable** (Discovery · Pitch · Cierre · Navegar el final · Objeciones, cada uno con *Qué busco / Lo que digo / Qué escucho*) + **pipeline de 9 etapas y 9 conteos** (contactados → respondieron → sesiones → discovery→pitch → ofertas dichas → síes → pagados → diagnósticos → activos al mes 2). **La oferta Socio Fundador (retro 13-sep):** mes 1 **$2,500 + IVA** (diagnóstico de la operación + Compras con sus facturas + Alan semanal) · desde el mes 2 **$1,500 + IVA por sucursal, fijo para siempre** · **5 lugares** · sin compromiso · precio no negociable · única concesión: se paga hoy y, el día de la entrega, si el diagnóstico *"no te dice nada que te sirva"*, se regresa el mes 1 completo. **Guión ≠ doc 12:** el guión v0.6 conoce al ICP con el Mom Test; el doc 12 vende — dos instrumentos, no se fusionan.
- **El kit físico de venta (16-sep · `Product Strategy/_templates/*.html` → `_export/*.pdf` vía Chrome headless `--print-to-pdf`):** **`script-sesion-de-venta-fase-a`** (5 páginas, formato run-sheet con beats y cheat-sheet) · **`flyer-socio-fundador`** (media carta · logo charcoal · precio $2,500/$1,500 · QR con `segno` · pie *"© 2026 Zenet® · Marca registrada ante el IMPI · Programa limitado a los primeros Socios Fundadores"*) · **`kit-bienvenida-socio-fundador`** (2 páginas: fechas · qué mandar · cómo se paga · *"Lo que acordamos"* con las **reglas de tiempo y cobro** — el mes 1 corre desde la entrega · fecha de corte mensual · pago por adelantado con 5 días · pausa sin recargos · 30 días → lugar liberado). Logo vectorizado del EPS del brandbook → `Branding/_context/assets/logos/zenet-imagotipo-teal.svg`. CLABE/beneficiario y facturación quedan `[Provisional]` hasta el banco y el contador.
- **`mensajes-outreach-fase-a` v0.3** — el diagnóstico se paga, así que **el primer toque (#9) lleva el diagnóstico de ejemplo ADJUNTO y pide el café de 20 minutos** (*"cómo se vería el tuyo"*); el #7 (cinco facturas gratis) se acota a conseguir material del ejemplo — **supersede el ask #1 del 10-sep**. **`10-scorecard` v0.5** — **§2.2 el embudo de ventas** como tabla propia (9 conteos + Σ desde el 14-sep; validación y venta en el mismo doc, tablas distintas); la skill del domingo pide los nueve. **`02-programa` y `01-modelo` v0.1.1** — nota de campo con las **cinco desviaciones** de la oferta al canon (la 5ª: el acuerdo es el kit + *"va"* por WhatsApp, no MSA + DocuSign); gradúan a v0.2 con el primer SF pagado.
- **Marca: el registro de Zenet ante el IMPI fue OTORGADO (14-sep)** → `Zenet®` permitido en materiales (flyer y kit ya lo llevan; la landing cuando se toque). **Archivados 16-sep** en `_templates/_archive/`: `criterios-icp-y-design-partner-fase-a` · `guion-pitch-deck-v0.1` · `guion-reunion-canirac-fase-a`; la edición de campo del ICP salió de git a `_export/`. Bitácora **v0.8** (13-sep · sesión paralela). **Pendiente:** push del workspace · PDF del diagnóstico de ejemplo (dependencia del #9) · link Dub · CLABE.

**As of 2026-09-10:**

- **Landing v4 ✓ EN VIVO en `zenetapp.com` (tag `v4.0`) 2026-09-10** — 10 secciones en `index.html` (merge `--no-ff` v4→main · verificado en producción: `/` · `/demo` · `/privacidad` · `/hablemos` · OG = 200). Las nueve piezas de `lab/` se integraron en la página: **§2 El momento** sube desde §7 con el dominó, los dos datos y la gráfica de compras · **§3** cinco tarjetas + remate · **§4** copy nuevo, sin capturas (variante C: *§4 promete, §5 prueba*) · **§5 El camino** con **Etapa 1 nueva** (factura al chat → tus precios, 4 beats), **Etapa 2 nueva** (7 beats: análisis → hallazgos → recetas → inventario → equivalencias → sucursales), **Etapa 3** con el dominó y **puntos de progreso en las tres escenas** (defecto que hoy existe en producción: nadie sabe en qué beat va), y **la consulta CONSERVADA** como cierre · **§6 En concreto** (nueva, 5 tarjetas) · **§7** recalibrada al ICP (*Recuperas tu cabeza* entra, *Creces sin multiplicar el caos* sale) · §8-10 sin cambio. **284 KB la página completa** (todas las escenas son SVG + CSS, cero capturas), móvil verificado a 390 px y en el teléfono del fundador, meta/OG al marco de la v4, CSS muerto y `app-catalogos.png` fuera. **Gate levantado con una condición (10-sep):** el **PDF ya corre**; **Compras y WhatsApp no todavía** (el fundador los está terminando). **Decisión canónica: la narrativa puede describir el camino; las instrucciones solo pueden pedir lo que ya funciona** — la landing se publica tal cual (si preguntan en la mesa, *"eso entra esta semana"*), y **`/demo` paso 04 lleva la píldora *En construcción*** porque ahí se le pide al operador que mande una factura. **v4.1 pendiente:** retirar esa píldora · las capturas reales (entran sin mover el layout) · refrescar el caché de OG.
- **`/demo` v0.2 — re-encuadrada para la v4** (`SEO and Content/_context/02-demo/00-estrategia-y-copy.md`). Dejaba de ser cierto que fuera *"la primera etapa del camino"* (la Etapa 1 es ahora Compras + WhatsApp). **Corrección de secuencia del fundador:** Compras **no es la puerta del demo, es lo primero que se toca ya dentro** — así que entra como **paso 04** del proceso (*"le mandas tu primera factura por WhatsApp, el mismo día"*), no como bloque antes del ask de documentos. El reloj se acota a *"del primer documento a tu análisis: menos de 10 minutos"* · el **PDF que se lleva** entra en el paso 03 y en *Qué vas a ver* · la letra honesta pasa de *"primera etapa"* a **qué corre hoy / qué todavía no** · hero nuevo (**"Verás los números reales de tu operación — y qué te está costando no verlos"**, titular del fundador; se descartó *"cómo mejorarlos"* por el veto *Zenet extrae, nunca propone*) · y en la landing el enlace *"Mira cómo funciona →"* **baja de la Etapa 1 a la Etapa 2**.
- **Docs de la landing al día:** `00-estrategia` **v0.4** (D0 dos entradas · D1 cifras de contexto · arquitectura de 10 secciones) · `01-copy` **v0.5** (10/10 secciones, escritas y aprobadas una por una) · `02-prototipo-y-diseno` **v4.0** con **§G-bis, el build registrado**. **`mensajes-outreach-fase-a` v0.2**: el ask #1 pasa de *una llamada* a **"mándame fotos de tus últimas 5 facturas"** · **#7 nuevo** (el ask de la factura, con su regla: se paga en 48 h o no se manda) · **#8 nuevo** (conocido de conocido, doble ask) · **#5 redactado** (seguimiento de hilo frío · variante viva para Juanjo) · **#4 LinkedIn reencuadrado**: refuerzo o insiders, **nunca primer toque** con el perfil ③.
- **La fuente del 40% VERIFICADA — y corregida.** *"Restaurantes absorben alzas de hasta 40%…"* — **El Imparcial · Dinero · 12-ago-2026 · Diyeth Arochi**; la cifra la da **Ignacio Alarcón Rodríguez Pacheco, presidente de CANIRAC**, y es **NACIONAL, no de Tijuana** (nuestra atribución decía Tijuana: era falsa). Primer semestre 2026, sobre carne de res, lácteos, verduras, jitomate y **aguacate**. La nota además regala el mecanismo que §2 quería enseñar —*el menor consumo impide subir los precios del menú, así que el alza la absorbe el restaurante*— y tiene gemela local (El Imparcial Tijuana, 22-mar-2026). **Retirado por falta de fuente publicada: el *"6-20% de costos operativos"*** — la nota de CANIRAC Tijuana que lo respaldaría existe (Uniradio, 18-ago-2026, Rebeca Aguilar) y confirma lo cualitativo, pero no publica el porcentaje.
- **Tres reglas nuevas que salieron del build** (en el spec §G-bis y §H): **(1)** en las secciones de tarjetas (§3 · §4 · §6) **el cuerpo va corto y el remate en negritas** — cuando una tarjeta se ve "grotesca" o "muy junta", el problema es el texto, no la caja; **(2)** al mover CSS entre `lab/` y el index se copia **por bloque, nunca filtrando por selector** — el filtro pierde los `@media` y los `@keyframes` (un `@media (prefers-reduced-motion)` desenvuelto mató la animación del chat sin dar error); **(3)** `lab/_shot.html` = harness de captura del index (`?sel=#seccion` o `?full=1`), y en headless los estados finales se capturan con `--force-prefers-reduced-motion` — el movimiento solo se juzga en navegador real.

**As of 2026-09-08:**

- **El ICP de la Fase A se redefine — `04-go-to-market/11-icp-hipotesis-fase-a.md` v0.1 NEW (draft) + su edición de campo (PDF de 12 páginas).** Escrito desde el borrador del fundador con los aprendizajes de las 5 sesiones (jul-ago) y pulido bloque por bloque contra el canon. **Es un experimento, no el ICP canónico**: los docs de research (`04-segmentacion` · `05-perfil-de-cliente-ideal` · `00-customer-profile`) quedan intactos hasta que el experimento gradúe. **La apuesta:** operador **chico (1-2 sucursales · 3 aceptable), que carga la operación él mismo, joven y abierto a lo digital** — early adopters primero, restaurantes tipo Alma Verde después con disparador medible (2-3 SF del perfil ③/② con retención al Mes 3). **El eje de segmentación es el nivel de estandarización previa:** perfil ③ sin estandarizar = centro · ② alta cocina y ②b cafetería *con cocina propia* = adyacentes · ① estandarizado = carril de producto, no suma al N. **Separa dónde buscar (targeting: joven · canales tibios · CETYS · conocidos de conocidos · LinkedIn no como primer toque) de quién califica (fit: 3 eliminatorios + 6 calificadores).** **Regla de conteo §2.3:** suma al N quien pasa los 3 eliminatorios y ≥4 de 6 calificadores; el gerente/chef valida H1-H4, **solo el que paga valida H6 e intent**; N por cuenta. Aplicada: **Miguel entra (5/6) · Alma Verde sale (perfil ①) · JP fuera (formalidad)**. §7 = cómo se valida en la mesa (preguntas *"última vez"* por problema — dos del fundador: insumo que subió + qué hizo · rotación · el orden de 25 min · **el precio se dice, no se pregunta**, con el punto y no en el primer café · el ask · registro en H canon). Ventas $200K-2M/mes `[Hipótesis]` con H6 bajo vigilancia < $300K.
- **`10-scorecard-fase-a` v0.3** — recontado contra el ICP de Fase A: **1 conversación · 0 compromisos · 0 intent** (antes 1·1·1 con Alma Verde) · fila nueva del experimento *"hay de dónde extraer"* (0 de 0) · **H6 se lee en dos partes: pozo y precio** · intent-to-join = *conociendo el precio* · §1.1 con columna *qué hipótesis mide* · decisiones #1 y #2 cerradas. La skill `/scorecard-fase-a-semanal` sincronizada (regla §2.3 · H6 partida · pregunta por el precio).
- **Pipeline de producción de PDFs sin motor:** pandoc → HTML con CSS de marca (Onest/Hanken · teal) → **Chrome headless `--print-to-pdf`**. La edición de campo (`11-…_edicion-de-campo.md` → `_export/…pdf`) se regenera con dos comandos documentados en su cabecera; el PDF vive en `_export/` (gitignored).
- **Pendiente que este bloque deja escrito (`11` §8.3):** ~~realinear `_templates/guion-conversacion-y-demo-fase-a.md` a §7~~ ✅ **v0.6 (9-sep)** · `criterios-icp-y-design-partner` superseded para la Fase A **(sigue pendiente)** · `plantilla-runsheet` con los 9 checks **(sigue pendiente)** · ~~`mensajes-outreach` al ICP nuevo~~ ✅ **v0.2 (10-sep)**. Y la retro fijó el orden: **doc 11 → copy v0.5 / landing v4 → proceso de ventas v0.1.**

**As of 2026-09-02:**

- **Landing v3.3 ✓ EN VIVO (`zenetapp.com` · tag `v3.3`)** 2026-09-02 — **pasada de copy del fundador, sección por sección** (11 cambios + 1 retiro) + **animación nueva**. §3 pasa del mercado al costo humano: *"Lo más caro de tu operación no se compra. **Se carga.** / Nadie te ayuda a construir tu sistema, a mantenerlo ni a ejecutarlo."* · las 4 tarjetas reescritas (t1 cierra en *"corre con tu energía"* · t2 *"Todo se sostiene, pero nada se conecta"* · t3 *"años y dinero"* · t4 absorbe *"se lleva lo que sabía"*) · figuras con **brecha punteada** (§3 c3 y §6 c2 = la pérdida dibujada, sin signo de pesos). §5 **El camino baja a 3 etapas ciertas y CERO tags**: la Etapa 3 deja de ser automatización y pasa a **mantenimiento ante el cambio — el dominó** (5 beats: entra un cambio → turbulencia → Zenet recompone → "Al día"; el pill rota entre los 4 cambios del copy; **las piezas se sacuden y el núcleo no se mueve**). La automatización sale de §5 y sobrevive solo en el paso 04 del trato, con su tag — **el reloj del día se conserva en `lab/etapa3.html`**. §7 abre con el cambio de mercado, **sin una sola cifra**. Docs sincronizados: `00-estrategia` **v0.3** · `01-copy` **v0.4** · `02-prototipo-y-diseno` **v3.3**.
- **Landing v3.1 y v3.2 (registradas con retraso):** **`/demo` EN VIVO** 2026-08-11 (tag `v3.1` · antesala del análisis = Etapa 1 del camino · acceso cerrado · doc propio `SEO and Content/_context/02-demo/00-estrategia-y-copy.md` v0.1) · **`/privacidad`** · **Hablemos v0.4** 2026-08-20 (tag `v3.2` · formulario de 3 pasos con split panel de marca · 3 pasos escalados por intimidad · el reto en sus palabras = oro de VoC pre-conversación · QA con envío real).
- **Producto: la demo terminada, desktop y móvil** (semana 24-30 ago · Tasks 26/27/28 cerradas y verificadas contra producción) — el flujo de onboarding + análisis corre de punta a punta. **La deuda que dejó el gate de agosto quedó saldada**; falta correrlo con documentos reales de un operador.
- **Fase A — dos sesiones nuevas y dos bitácoras:** sesión **#4 Miguel Bahena** (12-ago · perfil 2 dueño-payer · la carga como dolor #1) y **#5 Alfonso Arellano** (27-ago · carril insider/conector · **primera demo completa mostrada a alguien de la industria**). Docs nuevos: `08-learnings-de-validacion` **v0.5** (una entrada por sesión) y `09-bitacora-semanal-fase-a` **v0.5** (retro semanal con scorecard H1-H6 · carril: **Notion ejecuta, la bitácora recuerda**).
- **⚠️ La señal central del track de validación (de la retro 24-30 ago, en palabras del fundador):** *"todo construido, nada validado"*. **El estancamiento es de ENTRADA, no de capacidad** — cinco canales tocados, cero respuestas · **3 semanas consecutivas sin conversaciones con operadores** · scorecard sin movimiento (1-2 de 5-8) · **perfil 3 en cero, 8ª semana**. La landing, la demo, el deck y los guiones están listos; lo que no avanza son las conversaciones.
- **Siguiente recomendado (de la propia retro):** ① **cebar el flujo** — el conocido del brunch de Alfonso (se evapora en días) · Juanjo (el mensaje lleva un mes) · Ana · ② **meter los documentos de Alma Verde a Zenet** (1ª corrida real del producto **y** el contenido del mensaje a Carlos) · ③ **la calculadora del punto** (media hora · para que HV-03 salga completo). **Deudas de doc CERRADAS 2026-09-05:** `06-argumentacion-de-valor-y-precio` **v0.2** (§6.1 reformulada con la raíz corregida — *copiar el criterio sí se puede: cuesta años y el mantenimiento no termina* · §6.6 y §6.7 sincronizadas con las 5 sesiones de campo) · **`01-landing-page/03-hablemos-estrategia-y-copy.md` v0.1 NEW** (el copy de `/hablemos` ya no vive solo en el HTML). La copy de `/demo` ya estaba documentada desde el 11-ago (la nota anterior era imprecisa).


**As of 2026-08-07:**

- **Landing v3 ✓ EN VIVO en producción (`zenetapp.com` · tag `v3.0`)** 2026-08-07 — rediseño completo sobre el copy v0.3 (arquitectura Sinek WHY→HOW→WHAT + raíz emocional "El caos no es el precio de tener un restaurante" + hilo transversal "cargar"). **9 secciones** (v2 tenía 11): Hero bandera · La realidad (5 filas verbo) · El trato (5 puntos HOW) · **El camino = pieza estrella** (3 etapas animadas beat-loop: documentos→análisis · áreas→estrella→sucursales · rutinas+línea del tiempo · + la consulta chat estilo WhatsApp) · Qué cambia · El momento · El fundador · La invitación · Cierre aurora. Sección WHY construida y RETIRADA en pantalla (repetía el hero). Regla v3: "los componentes hablan, la prosa estorba". Hero con colores sólidos compuestos (fix del look "lavado"). QA fase 3 completo (móvil + iPhone Safari + jitter CTA). **Gate de deploy LEVANTADO por decisión explícita del fundador** (los tags "En construcción" en Etapa 3/consulta = honestidad suficiente) — ⚠️ **deuda de producto prioritaria: onboarding + análisis corrido con docs reales** (el copy habla en presente). Docs sincronizados: `01-copy` v0.3 · `02-prototipo-y-diseno` **v3.0** (spec + QA + esquema SVG canónico anti-WebKit) · `00-estrategia-y-estructura` **v0.2** (narrativa v3 + historial v1→v2→v3) · v2 archivadas con date-prefix. Workspace pusheado a GitHub (`zenet-marketing` · 36 commits al día).
- **Lecciones técnicas del QA v3 (en spec §8):** NUNCA `vector-effect: non-scaling-stroke` con `preserveAspectRatio="none"` (WebKit/iPhone descompone la geometría) · esquema SVG canónico = viewBox + strokes en unidades + width/height 100% · harness de QA same-origin (`lab/_qa.html`) con freeze de beats · zoom por-sitio de Chrome/Safari causa falsas diferencias preview-vs-prod · avisar a Alan solo con deploy Ready + pestaña privada.
- **Siguiente recomendado:** Fase A mundo real — recordatorio a Victor por Miguel (kill date ~15-ago) · mensajes a Ana (San Miguel · perfil 3) y Juanjo · follow-up Carlos ~14-ago con análisis de sus docs (alimenta también la deuda del gate) · shipear onboarding + análisis (producto).

**As of 2026-07-22:**

- **Landing v2 ✓ EN VIVO en producción (`zenetapp.com` · tag `v2.0`)** 2026-07-22 — rediseño completo motivado por los learnings de la Sesión #1 de Fase A (Carlos · Alma Verde). Cambios mayores: **acento de marca terracota → teal profundo `#2E6E62`** (hover `#265C52` · diferenciación vs el mar-de-naranja del food-tech · "orden/zen sobre apetito" · decisión canónica en `Branding/03-visual-identity/02-color.md` **v2.0** · terracota v1 archivada) · **copy v2** (`01-copy` v0.2 · hero "Los grandes restaurantes operan con un equipo de especialistas. Ahora el tuyo también." · pains con fragmentación · §4 espejo con "un experto a tu lado") · **§3 partida en 3 movimientos** (El sistema · El equipo · El resultado) con fondos alternos · **animación "Cómo funciona"** (5 beats auto-avanzantes · metáfora ganadora "el tablero que corre solo" vs red-de-nodos · beat 5: el tablero se condensa en el núcleo teal = Zenet + Q&A estilo Jarvis) · **sección NUEVA "En acción"** (Así se ve un día con Zenet · 6 momentos = las 6 áreas · reloj de chips auto-avanzante + núcleo respirando · tag honesto "En construcción — junto a los primeros Socios Fundadores") · área nueva **Mantenimiento y permisos** en el tablero (dolor #1 de Carlos) · stepper de proceso · strip anti-POS ("Tu POS se queda. El caos se va.") · OG image + favicon regenerados en teal (la v1 decía "por el precio de una app" — claim retirado) · QA (hablemos.html seguía terracota → teal) · 11 secciones. Método: branch `v2` + iteración en `lab/` (color · stepper · howworks A/B) + merge `--no-ff` a main. Spec: `02-prototipo-y-diseno.md` **v2.0**.
- **Fase A en ejecución (2026-07-08→15):** Ola 1 enviada · **Sesión #1 presencial con Carlos (gerente · Alma Verde) 2026-07-14** — learnings: dolor #1 = protocolos/procesos internos personalizados + mantenimiento · anti-POS explícito ("No quiero que sea un POS" · V-035 en VoC) · operador sofisticado ya estandarizado = N=1 posible outlier (no pivotar ICP con una muestra) · ofrece intros (Alma + otros departamentos) · kit de validación construido (guión Mom Test · criterios ICP/DP · debrief template · PDFs/Word para iPad) · bitácora nueva `Product Strategy/_context/04-go-to-market/08-learnings-de-validacion.md` · pipeline: Pedro (contador) busca restaurante en su red · Victor → intro a **Miguel Bahena (Madre restaurante)**. Gestión visual en Notion cockpit `🎯 Zenet — Fase A`.
- **Strategic decisions 2026-07-20/22:** acento teal canónico (ver §4) · §4 de la landing sin animación (2 piezas animadas bastan) · disciplina "apilar en el tiempo, no en el espacio" para secciones largas · **gate de Fase A: compartir la landing v2 con Miguel ANTES del outreach** (por eso se construyó v2 primero).
- **Siguiente recomendado:** redactar el mensaje a Miguel (Fase A · vía Victor · la landing v2 ya está lista para compartirse) · Ola 2 de outreach · refrescar `00-estrategia-y-estructura.md` de la landing (v2 añadió "En acción" y partió §3) · variante de logo en teal (open de `02-color` v2.0).

**As of 2026-07-02:**

- **Landing page ✓ EN VIVO en producción (`zenetapp.com`)** 2026-07-02 — pasó de diseño a producción pública. Sitio estático: repo GitHub privado `zenet-landing` (en `02_Producto-y-Tech/`, **fuera de este workspace**) + Vercel (auto-deploy on push) + dominio `zenetapp.com` (Cloudflare · HTTPS) + formulario a **Formspree** (activo · correos llegando) + pulido de producción (OG tags/imagen · favicon · meta). Spec de diseño sigue en `SEO and Content/01-landing-page/02-prototipo-y-diseno` (fuente de verdad); el repo lleva solo el artefacto desplegable.
- **`04-go-to-market/06-plan-de-activacion-y-validacion-fase-a.md` v0.1 NEW** — plan operativo de la **Fase A** del reclutamiento de design partners = **fase de VALIDACIÓN pre-producto**. Dos capas: activación (a quién · canal · mensaje) + validación (**6 hipótesis** H1-H3 propuesta de valor · H4 compromiso · H5 canal · H6 pozo de valor · con test/señal/kill). Frame **Mom Test + revelado>declarado + kill criteria de antemano** · outreach mix digital+presencial-tibio · scorecard · gate a Fase B (V1+legal+validación).
- **Strategic decisions 2026-07-02:**
  - **Fase A = validación (no solo outreach)** · 6 hipótesis · Mom Test · revelado>declarado · gate a Fase B por pre-condición medible (validación + V1 + legal), no calendario · precio exacto diferido a Mes 3.
  - **Landing deploy = estático (Vercel) sobre `zenetapp.com`** · repo separado del producto · Next.js solo con trigger de producto (leads a Supabase / SEO / componentes compartidos).
- **Siguiente recomendado:** ejecutar Fase A (pipeline de prospectos + intros · setup tracking · primeros mensajes) · construir assets de Fase A (tarjeta de fundador con QR · mensajes por canal · scorecard) · O activar otro departamento Execution (Analytics · Email/CRM).
...
| **Branding / 03-visual-identity** | ✓ v2.0 color 2026-07-22 | **`02-color` v2.0: acento canónico = teal profundo `#2E6E62`** (hover `#265C52` · en producción con la landing v2 · **terracota #CC5536 descartado como acento** — v1 archivada · #FF653B descartado desde v1) · arquitectura térmica invertida (base cálida / acento frío) · `00-marco` v1.0 · `03-tipografia` v1.0 (**Onest+Hanken**) · `04-iconografia` v0.2 · `05-aplicacion` v0.2 · `01-logo` v0.1 · **`assets/logos/`** con el imagotipo vectorizado (teal · desde el EPS del brandbook · 16-sep) + los PNG del brandbook. Opens: refrescar referencias terracota en 00/03/04/05 · ilustración BoH full · modo oscuro · design system de producto |
...
| **Product Strategy / 03-oferta-y-pricing** | ✓ **v0.1 — 7 docs** (6/6 cerrados + 06 argumentación draft) | 6 docs cerrados 2026-05-29/06-01 (~68 pág): `00-marco` v1.0 · `01-modelo-comercial-y-terminos` v0.1 · `02-programa-socio-fundador-offer` v0.1 · `03-pricing-tiers-hipotesis` v0.1 (4 tiers $1,500/$1,999/$1,799 + Enterprise) · `04-viabilidad-economica-y-cogs` v0.1 (sensitivity matrix · MX cost advantage) · `05-wtp-validation-plan` v0.1 (triangulación Behavioral 50% + Qualitative 35% + PSM 15%). **+ `06-argumentacion-de-valor-y-precio` v0.2 draft (2026-09-05)** — cómo se defiende el costo en dinero sin inventar cifras: mapa de gastos → dónde toca Zenet · costo teórico vs real (**la varianza como lo recuperable**) · **el punto porcentual como unidad de conversación** · frontera honesta. **v0.2: §6.1 reformulada** (*el criterio funciona; copiarlo se puede — cuesta años y el mantenimiento no termina*) + §6.6 (Alma Verde = prueba de la ruta alterna) + §6.7 (evidencia de las 5 sesiones · falta el perfil 3). **+ `01-modelo` y `02-programa` con nota de campo v0.1.1 (16-sep):** las **cinco desviaciones** de la oferta Socio Fundador al canon (precio congelado en vez de descuento · mes 1 $2,500 · sin compromiso · devolución anclada a la entrega · el kit como acuerdo) — el canon no se reescribe; gradúan con el primer SF pagado |
| **Product Strategy / 04-go-to-market** | ✓ **v0.1 — 13 docs** (6 cerrados + Fase A + comunidad + 2 bitácoras + scorecard + ICP de hipótesis + proceso de ventas) | 6 docs cerrados 2026-06-01: `00-marco` v1.0 (7 frames · 9 frameworks · framework theatre anti-pattern · 18 decisiones) · `01-gtm-playbook-phase-1` · `02-digital-distribution-strategy` (**LinkedIn ONLY**) · `03-outreach-strategy` · `04-sales-motion-three-session-demo` · `05-channel-partner-strategy` (consultor #1 · contador #2 · demand-side first). **+ `06-plan-de-activacion-y-validacion-fase-a` v0.1** (6 hipótesis H1-H6 · Mom Test · kill criteria · gate a Fase B). **+ `07-plan-de-comunidad-y-eventos` v0.1 draft** (comidas/eventos curados problem-first en TJ como canal de acceso + peer proof · se formaliza con 2-3 relaciones ancla). **+ `08-learnings-de-validacion` v0.5** (una entrada por sesión de campo: dicho vs hecho · scores H1-H6 + fit ICP/DP · learnings N=1 · implicaciones · evidencia). **+ `09-bitacora-semanal-fase-a` v0.8** (retro semanal con formato fijo · carril: Notion ejecuta, la bitácora recuerda · **v0.6: los números se mudan al 10**). **+ `10-scorecard-fase-a` v0.1 NEW (2026-09-05)** — el marcador: **tablero del gate** (H1-H6 acumulado con evidencia citada · las 2 condiciones A→B · cobertura de los 3 perfiles) + **serie semanal** (9 semanas reconstruidas · 7 sin movimiento, 4 consecutivas · perfil 3 en cero · la entrada casi sin medir) · 4 decisiones de conteo abiertas · **v0.3 (8-sep): recontado contra el ICP de Fase A (1·0·0) · H6 en dos partes · #1 y #2 cerradas** · **v0.5 (16-sep): §2.2 el embudo de ventas** (9 conteos + Σ desde el 14-sep). **+ `11-icp-hipotesis-fase-a` v0.1 NEW draft (2026-09-08)** — el ICP que la Fase A valida desde septiembre: experimento con kill criteria · statement · 3 perfiles por estandarización previa · dónde buscar vs quién califica · **regla de conteo** · 5 problemas (la carga primero) · anti-ICP (9) · cómo se valida en la mesa (§7) · qué cambia en el gate · trazabilidad vs research. **+ `11-…_edicion-de-campo.md`** → PDF de 12 páginas para la mesa (desde el 16-sep vive en `_export/`, fuera de git). **+ `12-proceso-de-ventas-fase-a` v0.1 CERRADO (2026-09-15)** — la venta, del primer mensaje al Socio Fundador activo: script ensayable en 5 etapas · la oferta SF como se dice en la mesa · 11 objeciones · pipeline de 9 etapas + 9 conteos · artefactos y deuda (kit de venta ✅ 16-sep) |
...
| **Product Strategy / 06-experience-y-roadmap** | ✓ **1 spec v1.1 + 1 nota** | **`01-diagnostico-v2-spec.md` v1.1.2 (2026-09-23) — el rediseño del reporte de diagnóstico del onboarding, cerrado con el fundador bloque por bloque:** eje = las preguntas del dueño · **5 págs** (tu operación en una página · lo que sale de tus documentos I y II · tu camino en orden · el mes que viene y el detalle) · **v1.1: el área de Compras (§4.3)** — especialista propio, 5 datos del Excel de compras o facturas, barras *"de cada $100 que compras"* por proveedor, *mismo insumo dos precios*, nunca recomendar proveedor · **§7c onboarding v2** (4 pantallas · tabla de áreas · coordinador + especialistas + conversación + sintetizador) · **dos índices** (el sistema de la cocina · *qué tanto se puede enseñar sin una persona clave*) · regla de tres lecturas y de píldoras · tres orígenes de dato (documento · declarado · sector) · el espejo *"lo que nos dijiste y lo que vimos"* · camino en dos tiempos (capturas · rutinas) + mapa de la 1.0 · línea base y *"lo que se movió"* del mes 2 · plantilla con/sin documento · **§8 contrato de generación para los agentes · §9 el asistente del diagnóstico**. Mocks HTML en `_mock/diagnostico-v2/` (3 versiones: 5 · 5 · 4 págs); el de primer mes es el **diagnóstico de ejemplo de venta** (`_export/diagnostico-v2/Diagnóstico de ejemplo · Zenet.pdf`). Handoff al repo de producción. + `00-cs-as-agents-vision-nota.md` v0.1-nota (CS-as-Agents vision · NO Phase 1) |
| **— EXECUTION LAYER —** | | |
| **Analytics and Reporting** | Pending | |
| **Email and CRM** | Pending | |
| **SEO and Content** | ✓ **Landing v4 + /demo v0.2 EN VIVO** (`zenetapp.com` · tag `v4.0`) 2026-09-10 | **Landing v3.3 en producción (`zenetapp.com` · tag `v3.3`)** — narrativa Sinek + raíz "el caos no es el precio" · 9 secciones · pieza estrella "El camino" · regla "los componentes hablan". **v3.3 (2-sep): pasada de copy del fundador** — §3 pasa del mercado al costo humano (*"lo más caro de tu operación no se compra. Se carga."*) · el camino baja a **3 etapas ciertas y cero tags** (la Etapa 3 pasa de automatización a mantenimiento ante el cambio, con **la animación del dominó**) · §7 abre con el cambio de mercado, sin cifras. Antes: `v3.0` landing (7-ago) · `v3.1` **/demo** (11-ago) · `v3.2` **Hablemos v0.4 en 3 pasos** (20-ago). **v4 EN VIVO (10-sep · tag `v4.0`):** 10 secciones — El momento sube a §2 · 5 problemas · el trato sin capturas · el camino con Etapa 1 = Compras + WhatsApp, 7 beats en la Etapa 2, **puntos de progreso** y la consulta conservada · **En concreto** nueva · Qué cambia recalibrada. 284 KB · móvil ✓ · **gate: PDF + Compras + WhatsApp**. **5 docs sincronizados:** `00-estrategia-y-estructura` **v0.4** + `01-copy` **v0.5** + `02-prototipo-y-diseno` **v4.0** + `03-hablemos-estrategia-y-copy` **v0.1** + `02-demo/00-estrategia-y-copy` **v0.2**. Artefacto vivo = repo `zenet-landing` |
...
- **Acento de marca = teal profundo `#2E6E62`** (hover `#265C52`), canónico desde 2026-07-22 (landing v2 en producción). Razones: diferenciación vs el mar-de-naranja del food-tech · "orden/zen sobre apetito" (BoH no es food-facing) · sale de la familia teal del brandbook. **Terracota `#CC5536` descartado como acento** (v1 archivada; la calidez de marca vive en la base off-white/peach/glass — arquitectura térmica invertida: base cálida / acento frío). `#FF653B` descartado desde v1 (saturación + no-AA). Acento **contenido** (botones · subrayado · números · eyebrows · núcleo del motivo · chips). → `Branding/_context/03-visual-identity/02-color.md` v2.0
...
- **Landing = fuente de verdad del diseño** en spec markdown (`02-prototipo-y-diseno.md` **v3.3**); el artefacto vivo es el **repo `zenet-landing`** (GitHub + Vercel · producción `zenetapp.com`). Version control: git tags (`v1.0`/`v2.0`/`v3.0`/`v3.1`/`v3.2`/`v3.3`) + `lab/` para experimentos + `_archive/` en los specs. → `SEO and Content/_context/01-landing-page/02-prototipo-y-diseno.md`
- **Reglas de contenido de la landing (canónicas desde 2026-09-02 · cifras actualizadas 2026-09-09):** **cifras de impacto de Zenet = nunca** hasta la primera curva real de un Socio Fundador · **cifras de contexto del sector = solo en la sección *El momento* de la v4, máx. 2-3, con fuente y año, que eduquen el mecanismo del operador** (la v3.3 en vivo sigue con cero cifras; la regla se reabrió por D0: la landing pasa a ser primer contacto digital sin Alan presente — `00-estrategia` bloque v0.4) · **cero colores de alerta en las superficies de marketing** — landing · `/demo` · edición de campo — (la tensión se marca con peach `#F4DED0` y se apaga al volver la calma teal; **acotada 2026-09-09:** no gobierna la app — su design system vive en el repo de producción — y las pantallas del producto se muestran en la landing tal como son, sin retocar) · **nunca "falta de X"** (su sistema funciona: se sostiene con él; decirle inestable/desordenado/ineficiente es ataque a identidad) · **los agentes no se nombran** (Zenet es uno, también en las animaciones). → `SEO and Content/_context/01-landing-page/00-estrategia-y-estructura.md` §4
...
### Equipo, capital y estructura (Fase A · 2026-08-30)

- **NO contratar ingeniería ahora.** El cuello de botella no está en producto (V1 va adelante, la validación va atrás). 1-2 ingenieros = 10-30× el burn actual, con cero LOI. Se revisa cuando la validación desbloquee, no antes. → `Product Strategy/_context/04-go-to-market/09-bitacora-semanal-fase-a.md`
- **NO cofundador ahora — escalera en su lugar.** Cuesta 20-50% (más que una ronda pre-seed completa, 10-20%), es la decisión más irreversible, y el hueco real —las conversaciones— es founder-led por naturaleza. → misma fuente
- **Asesor: Juanjo Gutiérrez es el candidato.** Construyó tech de logística en TJ y la escaló a España, **y vendió software B2B a restaurantes y cafés en Tijuana** — la motion exacta que hoy no avanza. → misma fuente


### ICP y validación (Fase A · 2026-09-08)

- **El ICP de la Fase A es un experimento explícito, no el canon.** Operador chico (1-2 sucursales · 3 aceptable) que carga la operación él mismo, joven y abierto a lo digital · perfil ③ sin estandarizar como centro, ② y ②b (cafetería con cocina propia) adyacentes, ① estandarizado fuera (carril de producto). Los docs de research no se editan hasta que gradúe (5-8 conversaciones registradas). → `Product Strategy/_context/04-go-to-market/11-icp-hipotesis-fase-a.md` §0 · §1 · §10
- **El eje de segmentación es el nivel de estandarización previa, no el número de sucursales.** → `11` §1.2
- **Dónde buscar ≠ quién califica.** Joven/abierto/canal es targeting — necesario para entrar, no suficiente para contar. Fit = 3 eliminatorios (formal · cocina propia · problema estructural) + 6 calificadores (carga · siente el número · hay algo que extraer · delega la carga no el criterio · ventas $200K-2M · decide solo). → `11` §2
- **Regla de conteo del N:** 3 eliminatorios + ≥4 de 6 calificadores + *quién valida qué* (gerente/chef valida H1-H4; solo el que paga valida H6 e intent) · N por cuenta · lo no sondeado cuenta como ausente. Umbrales = hipótesis, se afinan con 3 sesiones. → `11` §2.3
- **"Hay algo que extraer" es calificador, no eliminatorio** — es la hipótesis que el experimento prueba; su falla repetida (3×) es kill signal, no exclusión silenciosa. → `11` §0.3 · §2.3
- **El precio se dice, no se pregunta.** No en el primer café · nunca se evade · siempre con el argumento del punto. H6 se lee en dos partes: *pozo* (¿le cuesta hoy?) y *precio* (¿acepta el ancla al oírlo?). Un *"pagaría"* sin precio dicho no es dato. → `11` §7.7 · `10-scorecard` §1.2
- **Anti-ICP de campo (3 nuevos):** números conformes · nada que extraer · dice que sí y no da. → `11` §6
- **Cuándo subir de segmento:** 2-3 Socios Fundadores del perfil ③/② activos con retención al Mes 3 — pre-condición, no fecha `[umbral por fijar]`. → `11` §0.4
- ~~**El ask #1 de campo son las facturas por WhatsApp, no la reunión**~~ **(2026-09-10 · superseded 2026-09-16 — ver bloque *Ventas*: el diagnóstico se paga, así que el primer toque no pide facturas).** Es lo único que un operador del perfil ③ puede dar **sin preparar nada** —ya existen en su teléfono— y devuelve un número suyo (*qué subió y cuánto pesa al mes*). Regla dura: **se paga en 48 h o no se manda**; un ask que no se paga quema la relación. Cinco facturas, no *"tus facturas"*: un número cerrado se cumple hoy, un pedido abierto se posterga. → `Product Strategy/_templates/mensajes-outreach-fase-a.md` v0.2 #7 · `11` §7.8
- **`/demo` no se nombra como una etapa del camino** (2026-09-10). Es la **antesala del análisis** y la puerta a la app; nombrar etapas ahí obliga a re-sincronizar la página cada vez que el camino se reordena — pasó con la v4 (el análisis se movió de la Etapa 1 a la 2). Y **Compras no es la puerta del demo: es lo primero que se toca ya dentro** — va como paso 04, no antes del ask de documentos. → `SEO and Content/_context/02-demo/00-estrategia-y-copy.md` v0.2 decisiones #9 y #12

### Ventas (Fase A · 2026-09-15/16)

- **La oferta Socio Fundador es un experimento sobre el canon comercial, con cinco desviaciones registradas:** (1) $1,500 = precio de lista **congelado para siempre**, no 20-30 % de descuento · (2) mes 1 a **$2,500 + IVA** con el diagnóstico, por adelantado — no es setup fee, es el mes con Alan adentro y el compromiso revelado que H4 pedía · (3) **sin compromiso** de 90 días — el candado es el precio de fundador, que se pierde al salir · (4) **devolución sin plazo, anclada a la entrega** del diagnóstico, no a una ventana de 7 días · (5) **el acuerdo es el kit de bienvenida** con *"va"* por WhatsApp o firma en papel, no MSA + DocuSign. El canon no se reescribe; cada desviación se lee en una columna del embudo y gradúan a v0.2 con el primer SF pagado. → `03-oferta-y-pricing/02-programa-socio-fundador-offer.md` y `01-modelo-comercial-y-terminos.md` (nota de campo) · `12-proceso-de-ventas-fase-a.md` §3
- **El diagnóstico se paga antes de recibirse → el primer toque no pide facturas ni promete un número gratis.** Manda el **diagnóstico de ejemplo adjunto** y pide el café de 20 minutos (*"cómo se vería el tuyo"*). Pedir facturas gratis regala lo que se cobra. → `_templates/mensajes-outreach-fase-a.md` v0.3 #9 · `12` §1
- **Guión ≠ proceso de ventas.** El guión (`guion-conversacion-y-demo` v0.6) conoce al ICP con el Mom Test — dolores y aspiraciones, sin venta; el doc 12 vende. Dos instrumentos; no se fusionan ni se apuntan como continuación. → `12` §0.3
- **Validación y venta viven en el mismo scorecard, en tablas distintas:** §2.1 la serie de hipótesis (H1-H6) · §2.2 el embudo (9 conteos + Σ). **Conteos, no porcentajes** — con cinco lugares una tasa es ruido; lo que informa es en qué columna se para la gente. → `10-scorecard-fase-a.md` §2 · `12` §6.3
- **Reglas de tiempo y cobro del Socio Fundador:** el mes 1 corre desde la entrega del diagnóstico · fecha de corte mensual · pago por adelantado con 5 días de margen · si no llega, pausa sin recargos · 30 días → el lugar se libera y el precio de fundador se pierde. Facturación `[Provisional]` hasta el contador. → `12` §4 · kit de bienvenida p.2
- **Marca registrada:** el IMPI otorgó el registro de Zenet (2026-09-14) → `Zenet®` en materiales. Dominio canónico `zenetapp.com`, sin cambios antes del primer Socio Fundador. → memoria `dominio-y-marca-zenet`

### Diagnóstico v2 (producto · 2026-09-21 · Compras 2026-09-23)

- **El reporte de diagnóstico se organiza por las preguntas del dueño, no por los módulos de Zenet**, con dos índices (cocina · gente), tres orígenes de dato y sin gramática de déficit. Cifras en pesos = *lo que vale un punto* y *lo que hoy estás arriba del techo*; nunca una "brecha $0". → `Product Strategy/_context/06-experience-y-roadmap/01-diagnostico-v2-spec.md` §1 · §3
- **Solo se numera lo que ya corre en Zenet**; la 1.0 completa se muestra como mapa narrativo "en construcción con los primeros Socios Fundadores", sin fechas. Misma regla que la landing. → spec §5
- **"Lo que se movió desde tu línea base", nunca "ahorros con Zenet"**; solo desde la 2ª corrida y nunca en el ejemplo de venta. **La comparación con el precio de Zenet no va en el documento**: la dice Alan en la mesa. → spec §3.5 · §6
- **El asistente del diagnóstico** aclara y no propone (no precios, no proveedores, no menú); no inventa; no se nombra como agente. → spec §9
- **Compras es un área propia del reporte, con su especialista en el onboarding** (23-sep): lee el Excel de compras o las facturas del dueño y muestra cobertura · concentración por proveedor (*"de cada $100 que compras"*) · lo que se movió en % y en pesos al mes · *mismo insumo, dos precios* · quién subió más. **Es dato, nunca recomendación de proveedor**; con un solo mes no hay variación. Con todos los documentos el reporte pasa a 5 páginas; sin compras ni números sigue en 4. → spec §4.3 · §8.2 · §8.3 regla 9
...
- **Fase A del reclutamiento de design partners = fase de VALIDACIÓN (no solo outreach).** 6 hipótesis canónicas: H1-H3 propuesta de valor · H4 compromiso revelado · H5 fit de canal · H6 pozo de valor/must-have (el "QUE pagarían", no el cuánto). Disciplina: **Mom Test** · **revelado > declarado** · kill criteria de antemano · bajo N = cualitativo. Outreach **mix digital + presencial-tibio** (in-person primary MX). **Gate a Fase B = validación pasada + V1 demostrable + legal listo** (pre-condición medible, NO calendario). Precio exacto diferido a Mes 3. → `Product Strategy/_context/04-go-to-market/06-plan-de-activacion-y-validacion-fase-a.md`
...
├── CLAUDE.md                               # This file
├── README.md
├── .claude/commands/                       # /update-claude — audita este archivo contra el filesystem
│
...
│   │   └── assets/                         logos/ (imagotipo teal SVG + PNGs del brandbook)
│   ├── _sop/                               empty
│   ├── _templates/                         empty
│   └── skills/                             empty
│
...
│   │   │   └── _archive/
│   │   ├── 03-oferta-y-pricing/            ✓ v0.1 — 7 docs (00-05 cerrados + 06 argumentación de valor y precio · draft) + _export/
│   │   │   └── _archive/
│   │   ├── 04-go-to-market/                ✓ v0.1 — 13 docs (00-06 + 07 comunidad · 08 learnings · 09 bitácora · 10 scorecard · 11 ICP de hipótesis · 12 proceso de ventas) + _tracking/ + _export/ (edición de campo)
│   │   │   └── _archive/
...
│   │   │   └── _archive/
│   │   └── 06-experience-y-roadmap/        ✓ 1 spec v1.1 (diagnóstico v2 · con Compras · handoff a producción) + _mock/diagnostico-v2/ + 1 nota (CS-as-agents vision)
│   │       └── _archive/
│   ├── _sop/                               empty
│   ├── _templates/                         6 (guión · mensajes · plantilla-runsheet · script de venta · flyer · kit de bienvenida) + _archive/ (3 · 16-sep) + _export/ (PDFs, gitignored)
│   └── skills/                             empty
│
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
│
├── Analytics and Reporting/                # pending — mismo patrón: _context/ · _sop/ · _templates/ · skills/
├── Email and CRM/                          # pending — mismo patrón
│
├── SEO and Content/                        # ✓ ACTIVO
│   ├── _context/
│   │   ├── 01-landing-page/                ✓ v4.0 EN VIVO — 4 docs (00 estrategia v0.4 · 01 copy v0.5 · 02 prototipo v4.0 · 03 hablemos v0.1)
│   │   │   └── _archive/
│   │   └── 02-demo/                        ✓ v0.2 — 1 doc (estrategia y copy de /demo · en vivo con el tag v4.0)
│   ├── _sop/ · _templates/ · skills/       empty
│
└── Social Media Content Creation/          # ✓ v0.1 — _context/ 7 docs · _templates/ 4 · _sop/ 2 · skills/ empty
```
...
**Current state (2026-09-05):** **2 skills implementados** — ambos viven en `.claude/commands/` (los `skills/` de departamento son placeholders; Claude Code solo lee `.claude/commands/`). Contrato común: **proponen y esperan OK explícito** · no corren git ni commitean.

- **`/update-claude`** — audita el filesystem contra este CLAUDE.md, detecta desfases (status · versión · docs nuevos · carpetas · skills) y propone cambios sección por sección. Source: `.claude/commands/update-claude.md`.
- **`/scorecard-fase-a-semanal`** (NEW 2026-09-05) — el ritual del domingo, **después de escribir la retro** en `09-bitacora`. Lee la retro más reciente + las entradas de `08-learnings` de esa semana + el estado de `10-scorecard-fase-a` · extrae sesiones (y propone su clasificación), cambios H1-H6 (traduce mapas V/T al canon H y lo marca), cambios del gate · **pregunta solo lo que falta** (máx. 4: mensajes enviados→respondidos · ¿suma al N? · compromiso cumplido / intent · legal) · propone la fila nueva de §2 + el diff de §1 + la línea `📊` para la bitácora + bumps de frontmatter · aplica solo con OK. **Desde el 8-sep** aplica la regla de conteo de `11-icp-hipotesis-fase-a` §2.3 al preguntar *¿suma al N?*, lee H6 en dos partes (pozo · precio) y pregunta si se dijo el precio. **Desde el 16-sep (scorecard v0.5) propone DOS filas:** la de hipótesis (§2.1) y la del embudo de ventas (§2.2 · los nueve conteos como deltas de la semana + la Σ recalculada); pregunta los nueve en un solo bloque. **Nunca inventa un número:** lo que falta se pregunta; lo que sigue faltando queda `s/r`. Inputs: `$ARGUMENTS` opcional (rango de semana o pista). Source: `.claude/commands/scorecard-fase-a-semanal.md`.
...
| Modelo comercial y términos (estructura comercial cliente-facing · pure subscription · 90-day commitment + annual prepay · ventana cancelación 7 días + Day 8 primer cargo · self-serve in-app cancellation · click-wrap/e-signature contract · 12 anti-positions · 5 compliance anchors · **nota de campo 16-sep: las 5 desviaciones de la oferta SF**) | ✓ active v0.1.1 | `Product Strategy/_context/03-oferta-y-pricing/01-modelo-comercial-y-terminos.md` |
| Programa Socio Fundador offer (deal terms Fase 0 design partners · 3-tier graceful step-down lifecycle · GO/NO-GO Mes 3 · descuento 20-30% lock-in permanente · 10 beneficios enhanced · 6 reciprocidades · 10-step flow con intent confirmation + DocuSign signing Day 1 · 11 anti-patterns · **nota de campo 16-sep: la oferta en la calle y sus 5 desviaciones — precio congelado, mes 1 $2,500, sin compromiso, devolución en la entrega, el kit como acuerdo**) | ✓ active v0.1.1 | `Product Strategy/_context/03-oferta-y-pricing/02-programa-socio-fundador-offer.md` |
...
| Argumentación de valor y precio (cómo se defiende el costo de Zenet en dinero **sin inventar cifras** · mapa de gastos del restaurante → dónde toca Zenet · costo teórico vs costo real, **la varianza como lo recuperable** · **el punto porcentual como unidad de conversación** · frontera honesta de lo que NO se promete · **§6.1 v0.2: el criterio se puede copiar — cuesta años y el mantenimiento no termina** · §6.7 con la evidencia de las 5 sesiones) | 🚧 draft v0.2 | `Product Strategy/_context/03-oferta-y-pricing/06-argumentacion-de-valor-y-precio.md` |
...
| Plan de activación y validación — Fase A (plan operativo del reclutamiento de design partners · validación pre-producto · 2 capas activación+validación · 6 hipótesis H1-H6 con test/señal/kill · Mom Test + revelado>declarado + kill criteria de antemano · outreach mix digital+presencial-tibio · scorecard de verificación · gate a Fase B V1+legal+validación · decisiones abiertas · cross-doc deps) | ✓ active v0.1 | `Product Strategy/_context/04-go-to-market/06-plan-de-activacion-y-validacion-fase-a.md` |
| Plan de comunidad y eventos — nota Fase A.5 (comidas/eventos curados problem-first en TJ como canal de acceso + relaciones + peer proof, en paralelo a las conversaciones 1:1 · se formaliza con 2-3 relaciones ancla validadas) | 🚧 draft v0.1 | `Product Strategy/_context/04-go-to-market/07-plan-de-comunidad-y-eventos.md` |
| Learnings de validación — Fase A (registro durable **por sesión de campo**: contexto · **dicho vs hecho** · scores H1-H6 + fit ICP/DP · learnings marcados N=1 · implicaciones a Product Strategy / repo de producción / VoC · decisión · evidencia) | ✓ active v0.5 | `Product Strategy/_context/04-go-to-market/08-learnings-de-validacion.md` |
| Bitácora semanal — Fase A (retro **por semana**, la más reciente arriba, formato fijo: foco previo · logros · qué funcionó · learnings · decisiones · **los números viven en `10-scorecard`** · foco de la semana siguiente · carril: **Notion ejecuta, la bitácora recuerda**) | ✓ active **v0.8** | `Product Strategy/_context/04-go-to-market/09-bitacora-semanal-fase-a.md` |
| Scorecard — Fase A (**los números en un solo lugar** · §1 tablero del gate: H1-H6 acumulado con evidencia por sesión + las 2 condiciones A→B + cobertura de los 3 perfiles · §2 serie semanal, una fila por semana desde el 6-jul · definiciones de columna · 4 decisiones de conteo abiertas (¿Miguel suma al N? · ¿el soft-commit de Carlos es intent? · volver al canon H en los run-sheets · medir la entrada) · ritual del domingo · **regla anti-drift: el tablero se deriva de la serie + `08`** · **v0.5: §2.2 el embudo de ventas — los 9 conteos de `12` §6 como tabla propia, con Σ desde el 14-sep**) | ✓ active v0.5 | `Product Strategy/_context/04-go-to-market/10-scorecard-fase-a.md` |
| ICP de hipótesis — Fase A (**el perfil que la Fase A valida desde sep-2026 · experimento, no canon** · §0 qué apostamos / confirma / tumba / cuándo subimos · §1 statement + 3 perfiles por estandarización previa + tamaño y ventas · §2 dónde buscar vs quién califica + **regla de conteo** · §3 los 5 problemas con la carga primero · §5 lo que quiere + guardarraíl *"tú decides; Zenet lo carga"* · §6 anti-ICP (9) · **§7 cómo se valida en la mesa** (preguntas *última vez* · orden 25 min · el precio · el ask · registro) · §8 qué cambia en el gate · §10 trazabilidad vs research · etiquetas de origen por afirmación) | 🚧 draft v0.1 | `Product Strategy/_context/04-go-to-market/11-icp-hipotesis-fase-a.md` |
| ICP de hipótesis — edición de campo (versión de 12 páginas para la mesa: qué es la Fase A · las 6 hipótesis en plano · el experimento · el ICP · el filtro · dolor y deseo · a quién no · en la mesa I y II · el marcador · cierre · **fuente del PDF** en `_export/icp-fase-a_edicion-de-campo.pdf` · build: pandoc + Chrome headless, documentado en la cabecera) | 🚧 draft v0.1 · fuera de git desde el 16-sep | `Product Strategy/_templates/_export/` (antes en `04-go-to-market/`) |
| Proceso de ventas — Fase A (**la venta, del primer mensaje al Socio Fundador activo** · §0 encuadre: 8 etapas → el ICP que decide solo · orden de entrada red → referidos → cámaras → frío · §1 Discovery (mensaje #9 · apertura de 60 s · 7 preguntas con la *última vez* · paso = 3 de 5 + perfil ③/②) · §2 Pitch (espejo · puente · demo con el diagnóstico de ejemplo · anti-dashboard) · **§3 Cierre (la oferta SF como se dice · el punto · la concesión sin plazo · las 5 desviaciones)** · §4 Navegar el final (pago + docs + fecha en la mesa · reglas de tiempo y cobro) · §5 11 objeciones · **§6 pipeline de 9 etapas + 9 conteos** · §7 artefactos y deuda) | ✓ active v0.1 | `Product Strategy/_context/04-go-to-market/12-proceso-de-ventas-fase-a.md` |
| Kit de venta — Fase A (**`mensajes-outreach` v0.3** — #9 primer toque con el ejemplo adjunto · **`guion-conversacion-y-demo` v0.6** — conocer al ICP con el Mom Test · `plantilla-runsheet.html` · **`script-sesion-de-venta-fase-a.html`** → PDF de 5 páginas · **`flyer-socio-fundador.html`** → media carta · **`kit-bienvenida-socio-fundador.html`** → 2 páginas con *Lo que acordamos* · build: HTML con tokens de marca → Chrome headless `--print-to-pdf` · PDFs en `_export/`, gitignored) | ✓ 16-sep | `Product Strategy/_templates/` |
| **Diagnóstico v2 — spec del reporte** (el rediseño del reporte de diagnóstico del onboarding · por qué se rediseña (8 defectos del actual) · 10 principios · 5 páginas bloque por bloque con cálculo, origen y copy · dos índices · **§4.3 Compras (v1.1 · 23-sep): 5 datos · gráfica de proveedores · regla 9: nunca recomendar proveedor** · regla de píldoras · el espejo problema↔evidencia · camino en dos tiempos + mapa de la 1.0 · mes 2 "lo que se movió desde tu línea base" · variante sin documentos · 3 preguntas nuevas del onboarding · **contrato de generación para agentes** · **asistente del diagnóstico** · descartes con razón · `[Alan verifica]`) | ✓ active v1.1.2 | `Product Strategy/_context/06-experience-y-roadmap/01-diagnostico-v2-spec.md` · mocks en `_mock/diagnostico-v2/` |
...
| Color de marca (tokens hex · off-white dominante + **acento canónico teal profundo #2E6E62** + hover #265C52 + familia teal-400/mint · **descartados terracota #CC5536 (v2) y #FF653B (v1)** · arquitectura térmica base-cálida/acento-frío · disciplina acento-no-flood · en producción con la landing v2 · opens: variante logo teal · modo oscuro 🚧) | ✓ active v2.0 | `Branding/_context/03-visual-identity/02-color.md` |
...
| Landing page · estrategia y estructura (el trabajo de la página · **D0: dos entradas — primer contacto digital + página de confianza tras la visita en frío** · **D1: cifras de contexto permitidas solo en §2, con fuente y año** · audiencia = **ICP de la Fase A** · narrativa Sinek + raíz "el caos no es el precio" + hilo "cargar" · **arquitectura de 10 secciones (v4)** · reglas de contenido (cero cifras de impacto · cero colores de alerta en marketing · nunca "falta de X" · los agentes no se nombran) · historial v1→v4 · decisiones abiertas) | ✓ active **v0.4** | `SEO and Content/_context/01-landing-page/00-estrategia-y-estructura.md` |
| Landing page · copy por sección (**el copy de la v4 completo arriba — 10/10 secciones, aprobadas una por una** — y la v0.4 (= la v3.3 en vivo) intacta debajo · el porqué de cada cambio y los vetos de vocabulario · los recortes hechos en la integración · la idea "Etapa 4 · Resultados" aparcada con gate) | ✓ active **v0.5** | `SEO and Content/_context/01-landing-page/01-copy.md` |
| Landing page · prototipo y diseño (spec de diseño · repo `zenet-landing` + Vercel = artefacto vivo · tokens :root teal · Onest+Hanken · grid 12-col 1280 · las 9 secciones · **la pieza estrella: 3 escenas animadas + la consulta** · **la escena del dominó v3.3** · componentes/patrones · inventario de scripts · **QA: harness `lab/_qa.html` + `lab/_shot.html` (encuadra una sección del index) · esquema SVG canónico anti-WebKit · el CSS se mueve por bloque, nunca filtrando por selector · headless con `--force-prefers-reduced-motion` para estados finales, nunca para movimiento ni móvil** · Hablemos v0.4 en 3 pasos · assets · deploy · **§G-bis: el build de la v4 registrado**) | ✓ active **v4.0** | `SEO and Content/_context/01-landing-page/02-prototipo-y-diseno.md` |
| Página /hablemos · estrategia y copy (el destino de los 3 CTA de la landing · EN VIVO v0.4 · tag `v3.2` · **el copy verbatim de los 3 pasos** —panel · etiquetas · placeholders · legal · éxito— + las 8 decisiones canónicas + el payload que llega por correo + cómo se lee un envío contra los criterios de ICP *(propuesta, sin calibrar)* · el diseño vive en `02-prototipo` §9.b) | ✓ active v0.1 | `SEO and Content/_context/01-landing-page/03-hablemos-estrategia-y-copy.md` |
| Página /demo · estrategia y copy (**la antesala del análisis y la puerta a la app — ya NO "la Etapa 1 del camino"** · **v0.2 EN VIVO con el tag `v4.0`** (antes `v3.1`) · flujo documentos → análisis → **PDF** → *ya dentro*, la primera factura por WhatsApp (**paso 04**) · el reloj acotado a 01→03 · la letra honesta en *qué corre hoy / qué todavía no* · 13 decisiones canónicas · los 8 bloques) | ✓ active **v0.2** | `SEO and Content/_context/02-demo/00-estrategia-y-copy.md` |

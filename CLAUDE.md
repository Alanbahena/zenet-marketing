...
- **Siguiente recomendado (de la propia retro):** ① **cebar el flujo** — el conocido del brunch de Alfonso (se evapora en días) · Juanjo (el mensaje lleva un mes) · Ana · ② **meter los documentos de Alma Verde a Zenet** (1ª corrida real del producto **y** el contenido del mensaje a Carlos) · ③ **la calculadora del punto** (media hora · para que HV-03 salga completo). Pendientes de doc: `06-argumentacion-de-valor-y-precio` §6.1 (raíz corregida) · la copy de `/demo` y `/hablemos` solo vive en el HTML.
...
| **Product Strategy / 03-oferta-y-pricing** | ✓ **v0.1 — 7 docs** (6/6 cerrados + 06 argumentación draft) | 6 docs cerrados 2026-05-29/06-01 (~68 pág): `00-marco` v1.0 · `01-modelo-comercial-y-terminos` v0.1 · `02-programa-socio-fundador-offer` v0.1 · `03-pricing-tiers-hipotesis` v0.1 (4 tiers $1,500/$1,999/$1,799 + Enterprise) · `04-viabilidad-economica-y-cogs` v0.1 (sensitivity matrix · MX cost advantage) · `05-wtp-validation-plan` v0.1 (triangulación Behavioral 50% + Qualitative 35% + PSM 15%). **+ `06-argumentacion-de-valor-y-precio` v0.1 draft (2026-07-31)** — cómo se defiende el costo en dinero sin inventar cifras: mapa de gastos → dónde toca Zenet · costo teórico vs real (**la varianza como lo recuperable**) · **el punto porcentual como unidad de conversación** · frontera honesta. ⚠️ Su §6.1 ("funciona pero no se puede copiar") quedó invalidado por la corrección de la raíz de la landing — pendiente actualizar |
| **Product Strategy / 04-go-to-market** | ✓ **v0.1 — 10 docs** (6 cerrados + Fase A + comunidad + 2 bitácoras) | 6 docs cerrados 2026-06-01: `00-marco` v1.0 (7 frames · 9 frameworks · framework theatre anti-pattern · 18 decisiones) · `01-gtm-playbook-phase-1` · `02-digital-distribution-strategy` (**LinkedIn ONLY**) · `03-outreach-strategy` · `04-sales-motion-three-session-demo` · `05-channel-partner-strategy` (consultor #1 · contador #2 · demand-side first). **+ `06-plan-de-activacion-y-validacion-fase-a` v0.1** (6 hipótesis H1-H6 · Mom Test · kill criteria · gate a Fase B). **+ `07-plan-de-comunidad-y-eventos` v0.1 draft** (comidas/eventos curados problem-first en TJ como canal de acceso + peer proof · se formaliza con 2-3 relaciones ancla). **+ `08-learnings-de-validacion` v0.5** (una entrada por sesión de campo: dicho vs hecho · scores H1-H6 + fit ICP/DP · learnings N=1 · implicaciones · evidencia). **+ `09-bitacora-semanal-fase-a` v0.5 NEW** (retro semanal con formato fijo · scorecard H1-H6 · carril: Notion ejecuta, la bitácora recuerda) |
...
│   │   │   └── _archive/
│   │   ├── 04-go-to-market/                ✓ v0.1 — 10 docs (00-06 + 07 comunidad · 08 learnings de validación · 09 bitácora semanal) + _tracking/
│   │   │   └── _archive/
...
│   ├── _context/
│   │   ├── 01-landing-page/                ✓ v3.3 EN VIVO — 3 docs (00 estrategia v0.3 · 01 copy v0.4 · 02 prototipo v3.3)
│   │   │   └── _archive/
...
**Current state (2026-09-02):** **1 skill implementado** — `/update-claude` (vive en `.claude/commands/update-claude.md`, no en un `skills/` de departamento): audita el filesystem contra este CLAUDE.md, detecta desfases (status · versión · docs nuevos · carpetas · skills), propone cambios sección por sección y **espera OK explícito** antes de aplicar. No corre git ni commitea. Los `skills/` de cada departamento siguen vacíos.

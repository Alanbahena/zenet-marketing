...
*Next planned update: cuando se active siguiente subfolder Product Strategy `04-go-to-market/` v0.1 (4 docs pendientes: GTM playbook · outreach · sales motion three-session demo · channel strategy) O `06-experience-y-roadmap/` (import desde production repo) · cuando se redacte `05-capital-y-finanzas/00-funding-roadmap-y-milestones.md` v0.1 (Path A vs B framework formal) · cuando `02-financial-model-y-projections.md` mueva de pre-draft a v0.1 (5 triggers cumplidos · ver scaffold doc) · cuando se active otro departamento (Social Media LinkedIn productized · SEO/Content · Analytics) · cuando customer research transitions de `discovery-pre-PMF` a `design-partner-validation` etapa · cuando un new strategic decision belongs en section 4.*
